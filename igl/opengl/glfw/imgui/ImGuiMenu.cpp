...
IGL_INLINE void ImGuiMenu::draw_viewer_menu(igl::opengl::glfw::Viewer *viewer, std::vector<igl::opengl::Camera*> &camera,Eigen::Vector4i& viewWindow,std::vector<DrawInfo *> drawInfos)
{
  ...
  // Mesh
  if (ImGui::CollapsingHeader("Load Object", ImGuiTreeNodeFlags_DefaultOpen))
  {
  ...
  //Split Screen
  if (ImGui::CollapsingHeader("Split Screen", ImGuiTreeNodeFlags_DefaultOpen))
  {
      if (ImGui::RadioButton("one screen", &(viewer->split_screen_state), 0))
      {

      }
      if (ImGui::RadioButton("split x", &(viewer->split_screen_state), 1))
      {

      }
      if (ImGui::RadioButton("split y", &(viewer->split_screen_state),2))
      {
  ...
  if (ImGui::CollapsingHeader("Layers", ImGuiTreeNodeFlags_DefaultOpen))
  {
      ...
      for (int i = 0; i < (viewer->layers).size(); i++) {
          std::string s = std::to_string(i);
          s = "layer" + s;
          char const* pchar = s.c_str();
          if (ImGui::Checkbox(pchar, viewer->layers[i])) {//*viewer->layers[i] bool value
                  for (int j = 1; j < viewer->data_list.size(); j++) {
                      if (viewer->data_list[j]->layer == i) {
                          if (*viewer->layers[i] == 1) {
                              viewer->data_list[j]->UnHide();
                          }
                          else {
                              viewer->data_list[j]->Hide();
                          }
                      }
                  }
          }
      ...
      {
          (viewer->layers).push_back(new bool(false));
      }
  }
  ...
  {
      int temp = viewer->data_list[0]->GetMaterial();
          if (ImGui::RadioButton("sky", &(temp), 3))
          {
              viewer->data_list[0]->SetMaterial(3);
          }
          if (ImGui::RadioButton("yokuma", &(temp), 2))
          {
              viewer->data_list[0]->SetMaterial(2);
          }
  }
  //shapes picking
    if (ImGui::CollapsingHeader("shapes", ImGuiTreeNodeFlags_DefaultOpen))
    {
        for (int shapeindx = 1; shapeindx < viewer->data_list.size(); shapeindx++) {
            std::string s = std::to_string(shapeindx);
          char const* pchar = s.c_str();
          if (ImGui::Checkbox(pchar, &(viewer->data_list[shapeindx]->is_picked))) 
          {
  ...
  //change layer of picked
    if (ImGui::CollapsingHeader("Change layer", ImGuiTreeNodeFlags_DefaultOpen))
    {
        ...
        for (int i = 0; i < (viewer->layers).size(); i++) {
            ...
            if (ImGui::ListBox("##Layers", &viewer->layer_change_state, values)) {
            }

        if (ImGui::Button("Change ler"))
        {
            for (int shapeindx = 1; shapeindx < viewer->data_list.size(); shapeindx++)
            {
                if (viewer->data_list[shapeindx]->is_picked) {
                    viewer->data_list[shapeindx]->layer = viewer->layer_change_state;
                    if (viewer->data_list[shapeindx]->layer == viewer->layer_change_state) {
                        if (*viewer->layers[viewer->layer_change_state] == 1) {
                            viewer->data_list[shapeindx]->UnHide();
                        }
                        else {
                            viewer->data_list[shapeindx]->Hide();
                        }
                    }
                }
            }
...
//change texture of picked shapes 
    if (ImGui::CollapsingHeader("Change Texture", ImGuiTreeNodeFlags_DefaultOpen))
    {
        if (ImGui::RadioButton("plane", &(viewer->texture_change_state), 0))
        {

        }
        if (ImGui::RadioButton("grass", &(viewer->texture_change_state), 1))
        {
        ...
        if (ImGui::Button("Change tex"))
        {
            ...
            {
                if (viewer->data_list[shapeindx]->is_picked) {
                    viewer->data_list[shapeindx]->SetMaterial(viewer->texture_change_state);
                }
            }
        }
    }
...
//      core[1].align_camera_center(viewer->data().V, viewer->data().F); TODO: add function like this to camera
    ...
    //{
    //  core[1].snap_to_canonical_qwuaternion();
    //}
